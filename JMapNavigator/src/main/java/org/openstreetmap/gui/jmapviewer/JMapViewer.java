...
public class JMapViewer extends JPanel implements TileLoaderListener {
	...
	public final JMapNavigatorMapController MapController;
	 ...
	 *            specified coordinate
	 * @param zoom
	 *            {@link #MIN_ZOOM} &lt;= zoom level &lt;= {@link TileSource#getMaxZoom()}
	 */
	...
	public void setDisplayToFitMapElements(boolean markers, boolean rectangles, boolean polygons) {
		int nbElemToCheck = 0;
		if (markers && mapMarkerList != null) nbElemToCheck += mapMarkerList.size();
		if (rectangles && mapRectangleList != null) nbElemToCheck += mapRectangleList.size();
		if (polygons && mapPolygonList != null) nbElemToCheck += mapPolygonList.size();
	 ...
	 *
	 * @param mapPoint
	 *            relative pixel coordinate regarding the top left corner of the displayed map
	 * @return latitude / longitude
	 */
	...
	public Integer getRadius(MapMarker marker, Point p) {
		if (marker.getMarkerStyle() == MapMarker.STYLE.FIXED) return (int) marker.getRadius();
		else if (p != null) {
	...
	public Point getMapPosition(Coordinate coord) {
		if (coord != null) return getMapPosition(coord.getLat(), coord.getLon());
		else return null;
	}
	...
	public Point getMapPosition(ICoordinate coord, boolean checkOutside) {
		if (coord != null) return getMapPosition(coord.getLat(), coord.getLon(), checkOutside);
		else return null;
	}
	...
	/**
	 * Every time the zoom level changes this method is called. Override it in derived implementations for adapting zoom dependent values. The new zoom level can be obtained via
	 * {@link #getZoom()}.
	 *
	 * @param oldZoom
	...
	public void setTileSource(TileSource tileSource) {
		if (tileSource.getMaxZoom() > MAX_ZOOM) throw new RuntimeException("Maximum zoom level too high");
		if (tileSource.getMinZoom() < MIN_ZOOM) throw new RuntimeException("Minimum zoom level too low");
